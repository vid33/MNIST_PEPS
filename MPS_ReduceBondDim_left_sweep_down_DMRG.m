function [ B ] = MPS_ReduceBondDim_left_sweep_down_DMRG( A, B )

    B = MPS_TransformToLeftGauge(B);
    ...
    %B{1} = reshape(Q, d, size(Q,2)/d);
    B{1} = reshape(sign(R)*Q, d, size(Q,2)/d); %NB if leftmost R dropped so state normalised.
    B{1} = Contract({B{1}}, {[-2, -1]});
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%
