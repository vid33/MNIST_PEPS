function [ out ] = contractW1_W2( W1, W2, l_pos)
    ...
    if l_pos(1) == 1 && l_pos(2) == 1
        ...
        out{1,1} = reshape(out{1,1}, D1*D2, D1*D2);
    end
    ...
    if l_pos(1) == 1 && l_pos(2) == N
        ...
        out{1,N} = reshape(out{1,N}, D1*D2, D1*D2);    
    end
    ...
    if l_pos(1) == N && l_pos(2) == N
        ...
        out{N,N} = reshape(out{N,N}, D1*D2, D1*D2, digit_no, digit_no);
    else
        out{N,N} = Contract({W1{N,N}, conj(W2{N,N}) }, {[-1, -3, 1], [ -2, -4, 1]});
        out{N,N} = reshape(out{N,N}, D1*D2, D1*D2); 
    end
    ...
    for kk=2:(N-1)
        if l_pos(1) == 1 && l_pos(2) == kk
            ...
            out{1,kk} = reshape(out{1,kk}, D1*D2, D1*D2, D1*D2);
        end
        if l_pos(1) == N && l_pos(2) == kk
            ...
            out{N,kk} = reshape(out{N,kk}, D1*D2, D1*D2, D1*D2);
        end
    end
    ...
    for kk=2:(N-1)
        if l_pos(1) == kk && l_pos(2) == 1
            ...
            out{kk,1} = reshape(out{kk,1}, D1*D2, D1*D2, D1*D2);
        end
        if l_pos(1) == kk && l_pos(2) == N
            ...
            out{kk,N} = reshape(out{kk,N}, D1*D2, D1*D2, D1*D2);
        end
    end
    ...
    for kk=2:(N-1)
        for mm=2:(N-1)
            if l_pos(1) == kk && l_pos(2) == mm
                ...
                out{kk,mm} = reshape(out{kk,mm}, D1*D2, D1*D2, D1*D2, D1*D2);
            end
        end
    end
